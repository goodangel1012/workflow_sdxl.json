...
def list_male_voice_ids(api_key: Optional[str] = API_KEY) -> list:
    """
    List male voice IDs from Cartesia.
    
    Args:
        api_key (str, optional): Cartesia API key
    
    Returns:
        list: List of male voice IDs
    """
    if api_key is None:
        api_key = os.getenv("CARTESIA_API_KEY")
    ...
    try:
        client = cartesia.Cartesia(api_key=api_key)
        voices = client.voices.list()
        male_voice_ids = [voice.id for voice in voices if voice.gender == "masculine"]
        return male_voice_ids
    except Exception as e:
        raise Exception(f"Failed to fetch male voice IDs: {str(e)}") 
    
    
def list_female_voice_ids(api_key: Optional[str] = API_KEY) -> list:
    """
    List male voice IDs from Cartesia.
    ...
    try:
        client = cartesia.Cartesia(api_key=api_key)
        voices = client.voices.list()
        female_voice_ids = [voice.id for voice in voices if voice.gender == "feminine"]
        return female_voice_ids
    except Exception as e:
        raise Exception(f"Failed to fetch male voice IDs: {str(e)}") 
